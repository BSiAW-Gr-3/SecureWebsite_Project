...
function Forum() {
  ...
  // NEW: State to track if the selected chat is ready for WebSocket connection
  ...
  // 3. Create Chat logic - Modified: No automatic navigation
  const handleCreateChat = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchChats();
        // REMOVED: setSelectedChat(chatName); 
      } else {
    ...
    const checkStatus = async () => {
      try {
        ...
        if (isMounted && data.status === 'ACTIVE') {
          ...
          // Poll again in 1 second if not active
  ...
  // 5. WebSocket - Modified: Only connects if isChatReady is true
  useEffect(() => {
